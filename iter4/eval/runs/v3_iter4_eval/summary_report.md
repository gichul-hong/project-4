...
| :--- | :---: | :---: | :---: |
| **F1-Score (종합)** | **0.3590** | $\ge 0.850$ | 🟡 REVIEW |
| **Precision (정밀도)** | **0.2857** (14/49) | $\ge 0.800$ | 🟡 REVIEW |
| **Recall (재현율)** | **0.4828** (14/29) | $\ge 0.850$ | 🟡 REVIEW |
| **종류 분류 정확도 (Kind Acc)** | **14.3%** | $\ge 90.0\%$ | 🟡 REVIEW |
| **비동작/풋워크 오검출 (Non-Action FP)** | **8회** | $\le 3회$ | 🔴 과검출 발생 |
| **평균 타격 지연 시간 (Timing MAE)** | **135.9 ms** | $\le 50.0 ms$ | 🟡 REVIEW |
...
1. **비동작 구간(풋워크/휴식) 과검출 방지**:
   * 현재 풋워크/휴식 구간에서 총 **8회**의 오검출이 발생했습니다.
