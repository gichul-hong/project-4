...
> **목표**: Computer Vision 과제 평가에서 높은 점수를 위해, 기존 MediaPipe Hands 기반 게임에 CV/AI 기술을 추가.
...
| **3** | 3D 깊이(Z좌표) 펀치 분류 | 1-2h | 없음 | ★★★ | [ ] |
| **4** | 얼굴 사진 → 3D 아바타 머리 | 3-5h | 없음 | ★★★ | [ ] |
| **5** | 웹캠 AR 이펙트 (네온 글로우) | 1-2h | 낮음 | ★★★ | [ ] |
| **6** | 바디 세그멘테이션 AR 오버레이 | 4-6h | 중-높음 | ★★★★★ | [ ] |
| **7** | 실제 환경 사진 → 링 배경 | 1-3h | 없음 | ★★ | [ ] |

---

## 1. [최우선] LSTM ONNX 브라우저 실시간 추론

**현재 상태**: BiLSTM 모델 학습 완료(`boxing_lstm.pth`, 합성 데이터 100%), 하지만 **런타임에 미사용**. 클라이언트는 속도 threshold 휴리스틱만 사용 중.
...
**현재 상태**: 손 랜드마크 21개를 펀치 속도 계산에만 사용.
