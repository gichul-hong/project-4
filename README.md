...
* **실시간 Web Audio 음향 & 최후 1인 생존 방식**: 타격음, 바람소리, 복싱 링 공 소리 및 실시간 전투 통계 HUD (제한 시간 없음 — 마지막 1인이 남을 때까지)
...
| :--- | :--- | :--- | :---: |
| **👊 왼손 잽 (Left Jab)** | 왼손을 카메라를 향해 앞으로 빠르게 찌름 | 화면 왼쪽 글러브가 전방으로 돌진 | **5 HP** |
| **👊 오른손 스트레이트 (Right Cross)** | 오른손을 카메라를 향해 앞으로 빠르게 찌름 | 화면 오른쪽 글러브가 전방으로 돌진 | **6 HP** |
| **🌀 훅 (Left/Right Hook)** | 팔을 옆에서 안쪽으로 원을 그리며 휘두름 | 회전 타격 및 상대방 넉백 유발 | **7 HP** |
| **⬆️ 어퍼컷 (Uppercut)** | 주먹을 아래에서 위로 힘차게 올려침 | 하단에서 솟구치는 타격 | **8 HP** |
| **🛡️ 양손 가드 (Dual Guard)** | 양 주먹을 얼굴/가슴 앞으로 모음 | 반투명 홀로그램 에너지 실드 전개 | **피해 80% 방어** |
| **⚡ 장풍 필살기 (Energy Wave)** | 양손을 가슴에 모았다가 앞으로 방출 | 광역 에너지 폭발 공격 | **12 HP** |
