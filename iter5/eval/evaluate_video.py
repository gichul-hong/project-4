...
from velocity_filter import make_filter
...
# v7: 손목 궤적 스무딩 필터 (velocity_filter.py).
#   VELOCITY_FILTER: None | "kalman" | "savitzky_golay"
#     None 이면 기존 유한차분 그대로. kalman/savitzky_golay 이면 손목 3D 궤적을
#     먼저 스무딩한 뒤 그 좌표로 reach/elbow/속도를 계산한다.
#   KALMAN_Q/KALMAN_R: 등속 칼만의 process/measurement noise.
#   SG_WINDOW: Savitzky-Golay causal 창 크기(홀수 권장).
...
def arm_kinematics(st, sh, el, wr, shoulder_w, now_ms, filt=None):
    # 손목 궤적 스무딩: filt 가 있으면 손목 좌표를 먼저 스무딩하고, 그 좌표로
    # reach/elbow/속도를 모두 계산한다. 필터는 속도 차분 기준(과거 위치)과
    # 일관되도록 st.x/y/z 에도 스무딩된 좌표를 저장한다.
    dt = None if st.t is None else (now_ms - st.t) / 1000.0
    if filt is not None and dt is not None and 0.008 < dt < 0.4:
        sx, sy, sz = filt.update(wr.x, wr.y, wr.z, dt)
    else:
        sx, sy, sz = wr.x, wr.y, wr.z
    wr_s = _Point3(sx, sy, sz)

    reach = dist3(wr_s, sh)
    elbow = angle_deg(sh, el, wr_s)
    ...
    st.x, st.y, st.z = wr_s.x, wr_s.y, wr_s.z
    st.reach = reach
    st.t = now_ms
    st.last_speed = speed
